...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, IMDL
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(f"𝙿𝙰𝙶𝙴𝚂 {math.ceil(int(offset) / 6) + 1} / {math.ceil(total / 6)}", callback_data="pages"),
        ...
        btn.append(
            [
                InlineKeyboardButton("𝙱𝙰𝙲𝙺", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"𝙿𝙰𝙶𝙴𝚂 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
                InlineKeyboardButton("𝙽𝙴𝚇𝚃", callback_data=f"next_{req}_{key}_{n_offset}")]
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        a = await query.message.edit_text(
            text="▰▱▱▱▱▱▱▱▱▱ 10%"
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"𝙿𝙰𝙶𝙴𝚂 1/{math.ceil(int(total_results) / 10)}", callback_data="pages"),
